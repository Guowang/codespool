...
int process(int, int, vector<string>&);

int bfProcess(int, int, vector<string>&);

bool isSuperString(string&);
    ...
    for (int i = 0; i < K; i++) {
        ...
        for (;;) {
            ...
            for (int j = 1; j < rsize; j++) {
                int x = rand() % (10 - (str[j - 1] - 'a'));
                str[j] = str[j - 1] + x;
            }
            string tss = str.substr(0, rand()%rsize + 1);
            //cout<<tss<<endl;
            in.push_back(tss);
            if (in.size() == N)
                break;
        }
        int p = process(N, M, in), bfp = bfProcess(N, M, in);
        if (p != bfp) {
            cout<<i<<": p: "<<p<<" bf: "<<bfp<<endl;
            cout<<"input: "<<endl;
...
int process(int N, int M, vector<string>& in) {
    if (M == 0) {
        //cout<<1<<endl;
        return 1;
    }
        ...
        for (int i = 2; i <= M; i++) {
            ...
            for (int k = 3; k <= M; k++) {
                if (duplicates[k] != 0 && i >= k) {
                    //cout<<"r[i]: "<<results[i]<<" d[k]: "<<duplicates[k]<<" r[i-k]: "<<results[i-k]<<endl;
...
int bfProcess(int N, int M, vector<string>& in) {
    if (M == 0) {
        //cout<<1<<endl;
        return 1;
    }
